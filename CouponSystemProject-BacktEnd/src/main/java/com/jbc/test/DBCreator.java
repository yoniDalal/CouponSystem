...
public class DBCreator {
	...
	public static void createDatabase(AdminServiceIFC adminServ, CompanyServiceIFC companyServ,
			CustomerServiceIFC customerServ) throws AlreadyExistsException, MessageException, DoesNotExistByIDException, IsNullException {
		...
		for (int i = 0; i < coupons.length; i++) {
			companyServ.addCoupon(coupons[i],coupons[i].getCompany().getId());
		}

		/* creating customers */
		Customer[] customers = new Customer[21];
		...
		customers[20] = new Customer("Eden", "Bachner", "bachner@gmail.com", "0612");

		/* adding customers */
		for (int i = 0; i < customers.length; i++) {
			adminServ.addCustomer(customers[i], 1);
		}

		/* updating the previously added coupons and customers id */
		List<Coupon> couponList = adminServ.getAllCoupon();
		for (int i = 0; i < coupons.length; i++) {
			coupons[i] = couponList.get(i);
		}
		List<Customer> customerList = adminServ.getAllCustomers();
		for (int i = 0; i < customers.length; i++) {
		...
		for (int i = 0; i < customers.length; i++) {
			adminServ.updateCustomer(customers[i], 2);
		}
	}
}